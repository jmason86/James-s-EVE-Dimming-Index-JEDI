...
def test_duration():
    ...
    assert selects_first_subsequent_positive_zero_crossing(light_curve)
...
def selects_first_subsequent_positive_zero_crossing(light_curve):
    time_early = pd.Timestamp('2010-08-07 18:30:00')
    duration_seconds, duration_start_time, duration_end_time = determine_dimming_duration(light_curve.copy(),
                                                                                          smooth_points=10,
                                                                                          earliest_allowed_time=time_early)
    if (duration_seconds == 10560) and \
            (duration_start_time == pd.Timestamp('2010-08-07 18:59:11')) and \
            (duration_end_time == pd.Timestamp('2010-08-07 21:55:11')):
        return True
