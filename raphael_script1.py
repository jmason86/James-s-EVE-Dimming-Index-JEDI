...
jedi_config.verbose = True
jedi_config.logger_filename = 'generate_jedi_catalog'

jedi_config.init()
jedi_config.make_jedi_df()
...
# <-> minutes_since_last_flare > jedi_config.threshold_time_prior_flare_minutes
...
for flare_index in flare_index_range:
    ...
    jedi_config.jedi_df.at[flare_index, 'GOES Flare Start Time'] = jedi_config.goes_flare_events['start_time'][flare_index].iso
    jedi_config.jedi_df.at[flare_index, 'GOES Flare Peak Time'] = jedi_config.goes_flare_events['peak_time'][flare_index].iso
    jedi_config.jedi_df.at[flare_index, 'GOES Flare Class'] = jedi_config.goes_flare_events['class'][flare_index]
    ...
    if not os.path.isfile(processed_lines_filename) or not os.path.isfile(processed_jedi_non_params_filename):
        ...
        jedi_config.jedi_df.at[flare_index, 'Pre-Flare Start Time'] = preflare_df['window start'][preflare_map_idx[flare_index_rel]]
        jedi_config.jedi_df.at[flare_index, 'Pre-Flare End Time'] = preflare_df['window end'][preflare_map_idx[flare_index_rel]]

        preflare_irradiance_cols = [col for col in jedi_config.jedi_df.columns if 'Pre-Flare Irradiance' in col]
        jedi_config.jedi_df.at[flare_index, preflare_irradiance_cols] = preflare_df.iloc[preflare_map_idx[flare_index_rel], 2:]
        ...
        # eve_jedi.peak_match_subtract(jedi_config, eve_lines_event, jedi_row, flare_index, ion_tuples, ion_permutations)
        # # Profile 1 ends
    ...
    #     eve_jedi.light_curve_fitting(jedi_config, eve_lines_event, jedi_row, flare_index, uncertainty)
    #     # Profile 2 ends
...
#     eve_jedi.determine_dimming(jedi_config, eve_lines_event, jedi_row, flare_index)
#
#     # Profile 0 stops
